...
class TeamPrefix {
    fun onCommand (sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        // 인자 개수 확인 (/team prefix <teamname> <prefix>)
        if (args.size < 3) {
            sender.sendMessage("§c사용법: /team prefix <팀이름> <prefix>")
            return true
        }

        val board = Bukkit.getScoreboardManager().mainScoreboard
        val teamName = args[1]
        val prefix = args.slice(2 until args.size).joinToString(" ")
        ...
        // 3. prefix 추가
        try {
            targetTeam.prefix(Component.text(prefix))
            sender.sendMessage("§a${teamName} 팀의 접두사가 '${prefix}'(으)로 설정되었습니다.")
